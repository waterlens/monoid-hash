...
CRC_AINLINE uint32_t gf2p32_xpow(uint32_t a, uint32_t n) {
  if (n == 0)
    return 1;
...
CRC_AINLINE m_crc32c_t m_crc32c_combine(m_crc32c_t a, m_crc32c_t b) {
...
CRC_AINLINE m_crc32c_t m_crc32c_fold_bytes(const char *data, size_t len) {
  return (m_crc32c_t){crc32c(data, len), gf2p32_xpow(0x40000000, len * 8)};
}
